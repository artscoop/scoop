...
class RegistrationForm(forms_.ModelForm):
    ...
    # Champs
    username = forms_.SlugField(max_length=30, min_length=4, widget=forms_.TextInput(attrs={'size': 20}), label=_(u"User name"))
    email = forms_.EmailField(widget=forms_.TextInput(attrs={'size': 20}), label=_(u"Email"))
    email_confirm = forms.EmailField(label=_(u"Retype email"))
    password_confirm = forms.CharField(max_length=128, widget=forms.PasswordInput(), label=_(u"Retype password"))
    ...
    def clean_password(self):
        ...
        if not password or len(password) < RegistrationForm.PASSWORD_LENGTH_MIN:
            raise forms.ValidationError(_(u"Your password should be at least {min} characters long").format(min=RegistrationForm.PASSWORD_LENGTH_MIN))
        return password
    ...
    def clean_password_confirm(self):
        ...
        if 'password_confirm' not in self.cleaned_data or confirm != self.cleaned_data.get('password'):
            raise forms.ValidationError(_(u"The password must be identical in both fields."))
        return confirm
    ...
    def clean_email_confirm(self):
        ...
        if 'email_confirm' not in self.cleaned_data or confirm != self.cleaned_data.get('email'):
            raise forms.ValidationError(_(u"The email addresses must be identical."))
        return confirm
    ...
    def clean_name(self):
        ...
        if len(name) > RegistrationForm.FIRST_NAME_LENGTH_MAX:
            raise forms.ValidationError(_(u"Your first name must not exceed {max} characters").format(max=RegistrationForm.FIRST_NAME_LENGTH_MAX))
        return name
    ...
    def clean_username(self):
        ...
        if User.objects.filter(username__iexact=name):
            raise forms.ValidationError(_(u"This nickname is already in use."))
        ...
        if length > RegistrationForm.USERNAME_LENGTH_MAX or length < RegistrationForm.USERNAME_LENGTH_MIN:
            raise forms.ValidationError(
                _(u"Your nickname should be between {min} and {max} characters long.").format(min=RegistrationForm.USERNAME_LENGTH_MIN, max=RegistrationForm.USERNAME_LENGTH_MAX))
        return name.lower()
    ...
    def clean_eula(self):
        ...
        if 'eula' in self.cleaned_data and checked is False:
            raise forms.ValidationError(_(u"You must accept our EULA to proceed."))
        return True
...
class EULAForm(forms_.Form):
    ...
    eula = forms_.BooleanField(required=True, help_text=mark_safe(_(u"By ticking this checkbox, I abide to the {website} license agreement").format(website=settings.SITE_NAME)),
                               label=_(u"EULA"))
...
class AccountForm(RegistrationForm):
    ...
    original = forms.CharField(widget=forms.PasswordInput(), label=_(u"Original"))
