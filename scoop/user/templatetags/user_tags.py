...
def visitors_to(user, count=8, *args, **kwargs):
    ...
    :returns: un queryset d'utilisateurs qui ont un attribut "when"
...
def show_distance(user1, user2, unit='km', digits=2):
    """
    ...
    if not any([u.is_anonymous() or not u.profile.city for u in [user1, user2]]):
        distance = CoordinatesModel.convert_km_to(user1.profile.city.get_distance(user2.profile.city), unit)
        if distance > 0:
            unit = unit if unit in CoordinatesModel.UNIT_RATIO else 'km'
            return u"{}{}".format(intcomma(round_left(distance, digits)), unit)
    return ""
