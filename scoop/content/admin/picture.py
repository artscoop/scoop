...
class PictureAdmin(GenericAdminModelAdmin, AjaxSelectAdmin, AutoAuthoredModelAdmin, GenericModelUtil):
    ...
    fieldsets = ((_(u"Picture"), {'fields': ('author', 'license', 'content_type', 'object_id', 'image', 'title', 'description', 'weight')}), (_(u"Plus"), {'fields': ('audience',)}))
    change_form_template = 'admintools_bootstrap/tabbed_change_form.html'
    admin_integration_enabled = True

    @addattr(short_description=_(u"Resave selected pictures"))
    def resave(self, request, queryset):
        """ Sauvegarder à nouveau des images (update) """
        for picture in queryset:
            picture.save()

    @addattr(short_description=_(u"Delete picture thumbnails"))
    def delete_thumbnails(self, request, queryset):
        """ Supprimer les miniatures des images """
        for picture in queryset:
            picture.clean_thumbnail()
        self.message_user(request, _(u"The selected pictures' thumbnails have been cleaned."))

    @addattr(short_description=_(u"Delete picture(s) permanently"))
    def delete_pictures(self, request, queryset):
        """ Effacer définitivement les images """
        for picture in queryset:
            picture.delete(clear=True)
        self.message_user(request, _(u"The selected pictures have been deleted."))

    @addattr(short_description=_(u"Recalculate selected pictures dimensions"))
    def recalculate_size(self, request, queryset):
        """ Mettre à jour les dimensions des images """
        for picture in queryset:
            picture.update_size()
        self.message_user(request, _(u"Dimensions were recalculated successfully."))

    @addattr(short_description=_(u"Fix filename extension"))
    def fix_extension(self, request, queryset):
        """ Modifier l'extension du fichier si erronée """
        for picture in queryset:
            picture.set_correct_extension()
        self.message_user(request, _(u"Extensions were recalculated successfully."))

    @addattr(short_description=_(u"Become the selected picture(s) author"))
    def take_control(self, request, queryset):
        ...
        for picture in queryset:
            ...
            picture.save()
        self.message_user(request, _(u"The selected pictures are now owned by you."))

    @addattr(short_description=_(u"Remove ICC Color Profile"))
    def remove_icc(self, request, queryset):
        """ Retirer le profil de couleur ICC des images """
        for picture in queryset:
            picture.remove_icc()
        self.message_user(request, _(u"The selected pictures have been converted to basic sRGB."))

    @addattr(short_description=_(u"Optimize file size"))
    def optimize(self, request, queryset):
        """ Optimiser la taille des fichiers des images """
        for picture in queryset:
            picture.optimize()
        self.message_user(request, _(u"The selected pictures have been optimized."))

    @addattr(short_description=_(u"Enhance"))
    def enhance(self, request, queryset):
        """ Traiter l'image afin d'améliorer son apparence """
        for picture in queryset:
            picture.enhance(False)
        self.message_user(request, _(u"The selected pictures have been enhanced."))

    @addattr(short_description=_(u"Quantize"))
    def quantize(self, request, queryset):
        """ Réduire l'image à une palette + tramage """
        for picture in queryset:
            picture.quantize(False)
        self.message_user(request, _(u"The selected pictures have been quantized."))

    @addattr(short_description=_(u"Smart resize {number} percent").format(number=60))
    def liquid(self, request, queryset):
        """ Redimensionner à 60% en retirant les détails mineurs """
        for picture in queryset:
            picture.liquid(60, 60)
        self.message_user(request, _(u"The selected pictures have been smart resized."))

    @addattr(short_description=_(u"Increase contrast"))
    def contrast(self, request, queryset):
        """ Augmenter le contraste des images """
        for picture in queryset:
            picture.contrast(False)
        self.message_user(request, _(u"The selected pictures contrasts have been increased."))

    @addattr(short_description=_(u"Clone"))
    def clone(self, request, queryset):
        """ Dupliquer les images """
        for picture in queryset:
            picture.clone()
        self.message_user(request, _(u"The selected pictures have been cloned."))

    @addattr(short_description=_(u"Autocrop"))
    def autocrop(self, request, queryset):
        """ Découper automatiquement l'image (simple) """
        for picture in queryset:
            picture.autocrop()
        self.message_user(request, _(u"The selected pictures have been automatically cropped."))

    @addattr(short_description=_(u"Autocrop by feature detection"))
    ...
    @addattr(short_description=_(u"Rotate 90 degrees clockwise"))
    def rotate90(self, request, queryset):
        """ Pivoter l'image à 90° dans le sens des aiguilles """
        for picture in queryset:
            picture.rotate(90)
        self.message_user(request, _(u"The selected pictures have been rotated."))

    @addattr(short_description=_(u"Rotate 180 degrees"))
    ...
    @addattr(short_description=_(u"Rotate 90 degrees counter-clockwise"))
    ...
    @addattr(short_description=_(u"Mirror along the y axis (horizontally)"))
    def mirror_x(self, request, queryset):
        """ Effectuer une symétrie horizontale (autour de l'axe Y) """
        for picture in queryset:
            picture.mirror('x')
        self.message_user(request, _(u"The selected pictures have been mirrored."))

    @addattr(short_description=_(u"Mirror along the x axis (vertically)"))
    ...
    @addattr(short_description=_(u"Update the path of these pictures with current rules"))
    def update_picture_path(self, request, queryset):
        ...
        for picture in queryset:
            picture.update_path()
        self.message_user(request, _(u"The selected pictures paths have been updated."))

    @addattr(short_description=_(u"Delete empty media folders"))
    def clean_folders(self, request, queryset):
        """ Supprimer les répertoires vides """
        clean_empty_folders(settings.MEDIA_ROOT)
        self.message_user(request, _(u"Empty media folders have been deleted."))

    @addattr(short_description=_(u"Clear orphaned files and thumbnails"))
    def clean_everything(self, request, queryset):
        ...
        self.message_user(request, _(u"Orphaned files and thumbnails have been cleared."))
    ...
    @addattr(allow_tags=True, admin_order_field='image', short_description=_(u"Path"))
    ...
    @addattr(allow_tags=True, admin_order_field='updated', short_description=_(u"Upd."))
    ...
    @addattr(boolean=True, admin_order_field='deleted', short_description=_(u"Del."))
    ...
    def zip_upload(self, request):
        ...
        if form.is_valid():
            ...
            self.message_user(request, _(u"File has been processed."))
