...
class OpeningHoursMiddleware(MiddlewareBase):
    ...
    template = getattr(settings, "OPENING_HOURS_TEMPLATE", 'opening-hours.html')
    ...
    def now_in_timeframes(self, timeframes):
        ...
        for timeframe in timeframes:
            if timeframe[0] < timeframe[1]:
                return timeframe[0] <= hour < timeframe[1]
            elif timeframe[0] > timeframe[1]:
                return hour < timeframe[1] or hour >= timeframe[0]
        return False
        ...
        if user.is_authenticated():
            if not self.now_in_timeframes(OpeningHoursMiddleware.frames):
                if not user.is_staff and (self.groups and not user.groups.filter(name__in=self.groups).exists()):
                    ...
                    return do_render(request, template=OpeningHoursMiddleware.template, data={'hours': self.frames}, status_code=503,
                                     use_request=False)
        return response
