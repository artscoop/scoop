...
class CityTest(TestCase):
    ...
    def test_cities(self):
        ...
        liege = City.objects.find_by_name([50.63, 5.56], u"Liège")
        brussels = City.objects.find_by_name([50.85, 4.35], u"Brussels")
        ...
        self.assertFalse(liege.has_name(u"4000"), "The postal code 4000 should not be a name of Liège")
        self.assertTrue(brussels.has_code(u"1000"), "Brussels should have a code of 1000")
        self.assertEqual(brussels.type, "PPLC", "Brussels is a PPL/Capital")
        if 'fr' in languages:
            self.assertTrue(liege.has_name(u"Liège"), "Liège was expected, got {} instead.".format(liege))
            self.assertFalse(liege.has_name(u"Liége"), "Liége is not a name of Liège.")
            self.assertTrue(belgium.has_name(u"Belgique"), "Belgium should have the name Belgique")
            self.assertTrue(brussels.has_name(u"Bruxelles"), "Brussels should have the name Bruxelles")
        if 'en' in languages:
            self.assertTrue(liege.has_name(u"Liège"), "Liège was expected, got {} instead.".format(liege))
            self.assertTrue(belgium.has_name(u"Belgium"), "Belgium should have its own english name")
            self.assertTrue(brussels.has_name(u"Brussels"), "Brussels should have its own english name")
