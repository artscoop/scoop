...
def geonames_fill(countries):
    ...
    for country in countries:
        print u"Starting population for {}".format(country.name)
        gc.disable()
        ...
        if populate_cities(country) is True:
            ...
            print u"population was successfully run in {time:.02f} seconds.".format(time=time.time() - start_time)
        else:
            print _(u"population has failed. please investigate.")
        gc.enable()
    if renaming_allowed:
        gc.disable()
        start_time = time.time()
