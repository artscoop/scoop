...
class CountryAdmin(ViewOnlyModelAdmin):
    ...
    # Actions
    @addattr(short_description=_("Populate everything about this country"))
    def fill_all(self, request, queryset):
        """ Renseigner et arranger toutes les informations de pays """
        if settings.DEBUG is True:
            self.message_user(request, _("You can autopopulate this country by setting DEBUG to False."), level=messages.ERROR)
        else:
            getattr(geonames_fill, 'delay')(queryset)

    @addattr(short_description=_("Populate country from Geonames database file"))
    def populate_from_file(self, request, queryset):
        ...
        for item in queryset:
            ...
            if result == -1:
                self.message_user(request, _("Set settings.DEBUG to False before populating city tables."))
            elif result == -2:
                self.message_user(request, _("You must add a country with the correct code to populate."))

    @addattr(short_description=_("Rebuild city hierarchy in country"))
    def reparent(self, request, queryset):
        """ Reconstruire la hiérarchie des villes des pays """
        for item in queryset:
            getattr(reparent_cities, 'delay')(item)
        self.message_user(request, _("City tree successfully rebuilt."))

    @addattr(short_description=_("Populate country currencies"))
    ...
    @addattr(short_description=_("Rename every city with its short counterpart"))
    def short_name_cities(self, request, queryset):
        ...
        self.message_user(request, _("Every city in the database had its name successfully updated when required."))

    @addattr(short_description=_("Remove all cities from selected countries"))
    def purge_cities(self, request, queryset):
        ...
        for country in queryset:
            ...
            cities.delete()
        self.message_user(request, _("Cities from selected countries have been successfully deleted."))

    @addattr(short_description=_("Set selected countries to private"))
    def unpublish(self, request, queryset):
        """ Rendre des pays non publics """
        for country in queryset:
            country.public = False
            country.save()
        self.message_user(request, _("Countries have been made private."))
