...
class CityAdminModelAdmin(ViewOnlyModelAdmin, PicturedModelAdmin):
    ...
    # Actions
    @addattr(short_description=_(u"Delete every city in the database"))
    def flush_city(self, request, queryset):
        """ Supprimer les villes sélectionnées """
        queryset.delete()

    @addattr(short_description=_(u"Resave cities"))
    def resave(self, request, queryset):
        """ Réenregistrer les villes du queryset (update) """
        for city in queryset:
            city.save()

    @addattr(short_description=_(u"Fetch pictures for selected cities"))
    ...
    # Getter
    @addattr(allow_tags=True, admin_order_field='parent', short_description=_(u"Parent"))
    def get_city_parent(self, obj):
        ...
        if obj.parent is not None:
            ...
            return u"""<small class="muted">{feature}</small> <a href="?q={parent}">{name}</a>""".format(parent=parent_name, name=parent_name, feature=parent_type)
        else:
            return obj.country

    @addattr(allow_tags=True, admin_order_field='code', short_description=_(u"Code"))
    def get_city_code(self, obj):
        """ Renvoyer le code postal """
        return u"""{code}<small class="pull-right muted">{codes}</small>""".format(code=obj.get_code(), codes=len(obj.get_codes()) or "")
