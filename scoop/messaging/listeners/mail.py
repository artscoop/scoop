...
def default_mail_send(sender, mailtype, recipient, data, **kwargs):
    ...
    mailtype = MailType.objects.get_named(mailtype).get_categories() if isinstance(mailtype, str) else mailtype
    ...
    if must_receive or ((chose_to_receive or can_receive) and not restrict_online):
        MailEvent.objects.queue(recipient, mailtype, data, forced=must_receive)
